    ...
    protected void onTick() {
        ...
        .forEach(
            n -> {
                ...
                (outer.getLocal().getLocation() < n.getStart() && outer.getConfig().get(outer.getConfig().size()/2+1).getLocation() > n.getEnd()) ||
                (outer.getLocal().getLocation() > n.getEnd() && outer.getConfig().get(outer.getConfig().size()/2-1).getLocation() < n.getStart())) {
        ...
        // EOR
        // Central measure
        outer.getMeasures()[outer.getMeasures().length/2 - 1].stream()
        .filter(n -> n.getType() == Measure.CENTRAL)
        .forEach(
            n -> {
                MSI[] EORMsi = {new MSI(MSI.EOR), new MSI(MSI.EOR), new MSI(MSI.EOR)};
                newMeasures.add(new Measure(n.getID(), Measure.REACTION, outer.getLocal().getRoad(), outer.getLocal().getLocation(), EORMsi));
            }
        );
        ...
        .forEach(
            n -> {
                ...
                newMeasures.add(new Measure(n.getID(), Measure.REACTION, outer.getLocal().getRoad(), outer.getLocal().getLocation(), congestionLeadinMsi));
            }
        );
        ...
        for (int i = 1; i < outer.getMeasures().length/2; i++) {
            ...
            for (int j = 0; j < measures.size(); j++) {
                if (measures.get(j).getType() == Measure.CENTRAL) {
                    ...
                    newMeasures.add(new Measure(measures.get(j).getID(), Measure.REACTION, outer.getLocal().getRoad(), outer.getLocal().getLocation(), newMsi));
                }
            }
        }
        ...
        //         newMeasures.add(new Measure(n.getID(), Measure.REACTION, outer.getLocal().getRoad(), outer.getLocal().getLocation(), newMsi));
        //     }
        // );
